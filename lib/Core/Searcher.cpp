...
double WeightedRandomSearcher::getWeight(ExecutionState *es) {
  ...
  case Depth: 
    return es->weight;
  case InstCount: {
    uint64_t count = theStatisticManager->getIndexedValue(stats::instructions, es->getCurrentThread()->pc->info->id);
...
ExecutionState& RoundSearch::selectState() {
	index++;
...
void RoundSearch::update(ExecutionState *current,
	                          const std::set<ExecutionState*> &addedStates,
	                          const std::set<ExecutionState*> &removedStates) {
